...
//	ASSERT( pointsAround.size() == 4 );
	...
	for ( size_t i=0; i<pointsAround.size(); ++i )
	{
...
void CChinaMapDeviationFix::EarthToMars( double earthLongitude, double earthLatitude, double& marsLongitude, double& marsLatitude )
{
	CCorrectionPointDatabase::TRecordList pointsAround = 
		m_cpDatabase.GetEarthPointsAround( earthLongitude, earthLatitude, CORRECTION_POINTS_PRECISION );
...
//	ASSERT( pointsAround.size() == 4 );
	...
	for ( size_t i=0; i<pointsAround.size(); ++i )
	{
		if ( pointsAround[i].m_earthLongitude > earthLongitude )	// 中国是东经. 最西(左)端为东经73, 最东(右)为东经135度.
		{
			// 说明 校正点在 我这个点的 右(东)边
			if ( pointsAround[i].m_earthLatitude > earthLatitude )
			{
				// 说明校正点在 我这个点的 上(北)面.
				righttopIndex = i;
			}
...
//	ASSERT( pointsAround.size() == 4 );
	...
	// 用双线性插值求地球坐标经度.
	// 先在经度方向上线性插值.
	double topMarsLongitude = this->LinearInterpolate( 
		pointsAround[lefttopIndex].m_earthLongitude, pointsAround[lefttopIndex].m_marsLongitude,
		pointsAround[righttopIndex].m_earthLongitude, pointsAround[righttopIndex].m_marsLongitude,
		earthLongitude );
	double topEarthLatitude = this->LinearInterpolate(
		pointsAround[lefttopIndex].m_earthLongitude, pointsAround[lefttopIndex].m_earthLatitude,
		pointsAround[righttopIndex].m_earthLongitude, pointsAround[righttopIndex].m_earthLatitude,
		earthLongitude );

	double bottomMarsLongitude = this->LinearInterpolate( 
		pointsAround[leftbottomIndex].m_earthLongitude, pointsAround[leftbottomIndex].m_marsLongitude,
		pointsAround[rightbottomIndex].m_earthLongitude, pointsAround[rightbottomIndex].m_marsLongitude,
		earthLongitude );
	double bottomEarthLatitude = this->LinearInterpolate(
		pointsAround[leftbottomIndex].m_earthLongitude, pointsAround[leftbottomIndex].m_earthLatitude,
		pointsAround[rightbottomIndex].m_earthLongitude, pointsAround[rightbottomIndex].m_earthLatitude,
		earthLongitude );

	// 在纬度上对刚刚得出的两个插值点进行插值.
	marsLongitude = this->LinearInterpolate( 
		topEarthLatitude, topMarsLongitude, bottomEarthLatitude, bottomMarsLongitude, earthLatitude );

	//////////////////////////////////////////////////////////////////////////
	// 用双线性插值求地球坐标纬度.
	// 先在经度方向上线性插值.
	double topMarsLatitude = this->LinearInterpolate(
		pointsAround[lefttopIndex].m_earthLongitude, pointsAround[ lefttopIndex ].m_marsLatitude,
		pointsAround[righttopIndex].m_earthLongitude, pointsAround[righttopIndex].m_marsLatitude,
		earthLongitude );

	double bottomMarsLatitude = this->LinearInterpolate(
		pointsAround[leftbottomIndex].m_earthLongitude, pointsAround[leftbottomIndex].m_marsLatitude,
		pointsAround[rightbottomIndex].m_earthLongitude, pointsAround[rightbottomIndex].m_marsLatitude,
		earthLongitude );

	// 在纬度上对上面两个插值点再次进行插值.
	marsLatitude = this->LinearInterpolate( 
		topEarthLatitude, topMarsLatitude, bottomEarthLatitude, bottomMarsLatitude, earthLatitude );
}
