...
document.onkeydown= function (event){
	...
	if(evt.ctrlKey && evt.shiftLeft && evt.keyCode == Event.KEY_UP){
		if(logging__){
				JLog.clear();
				debugging__ = false;
				logging__ = false;
				Element.hide('logCtrl');
				Element.hide('log');
		}
				
	}

	if(evt.ctrlKey && evt.shiftLeft && evt.keyCode == Event.KEY_DOWN){
			debugging__ = true;
			logging__ = true;
			printf("::::: ReStart Debugging Output Window :::::");
			Element.show('logCtrl');
			Element.show('log');
	}

	if(evt.altKey && evt.keyCode ==Event.KEY_DOWN)
	{
		debugging__ = true;
		logging__   = true;
	}

	if(evt.altKey && evt.keyCode ==Event.KEY_UP)
	{
		debugging__ = false;
		logging__   = false;
	}
...
JLog.write = function(s) {
  ...
  if(toFile__){
	var res=$("CreatePicture").AddTextToLog(s,s.length);
  }
};
